...
static int migrate(sqlite3 *db) {
  const char *sql =
      ...
      // outlook007 邮箱池: 母邮箱 + 专属接码链接, 别名注册 + 单线程 in_use 锁
      "CREATE TABLE IF NOT EXISTS outlook_mailboxes ("
      "id INTEGER PRIMARY KEY AUTOINCREMENT,"
      "email TEXT NOT NULL COLLATE NOCASE UNIQUE,"
      "code_api_url TEXT NOT NULL,"
      ...
      "updated_at INTEGER NOT NULL DEFAULT (unixepoch())"
      ");"
      "CREATE INDEX IF NOT EXISTS idx_outlook_mailboxes_claimable "
      "ON outlook_mailboxes(is_active, in_use);"
      // 加入母号 workspace 的目标配置(全局默认, 可在前端改)
      "INSERT OR IGNORE INTO mail_settings(key,value) "
      "VALUES('outlook_workspace_id','63d426c3-e5bf-4846-8b9f-490637a55312');"
      "INSERT OR IGNORE INTO mail_settings(key,value) "
      "VALUES('outlook_join_mode','request');"
